window.ToastClass = ToastClass;

window.onload = function () {
	let componentStyle = document.createElement('style');
	let componentStyleHtml = 'div.alertATE-bg{left:0;top:0;opacity:0.3;background-color:black;width:100%;height: 100%;position:fixed;z-index:310;}' +
		'div.alertATE-wrapper{position:fixed;background-color:rgba(255,253,250,0.52);z-index: 320;left:40%;top:40%;padding:10px;border-radius: 6px;}' +
		'div.alertATE-content{background-color: white;border-radius: 4px;min-height: 50px;padding:10px;}' +
		'label.alertATE-msg{width:340px;padding-top:5px;font-size:14px !important;}' +
		'div.alertATE-icon{margin-right:12px;width:32px;height: 32px;float: left;}' +
		'div.alertATE-content div.icon-error{background-image:url(data:image/png;base64,);}' +
		'div.alertATE-content div.icon-success{background-image:url(data:image/png;base64,);}' +
		'div.alertATE-content div.icon-info{background-image:url(data:image/png;base64,);}' +
		'span.alertATE-closeBtn{background:url(data:image/png;base64,) no-repeat 50% 50% fixed;width: 10px;position: relative;margin: 12px 12px 12px 12px;padding: 8px 16px 8px 16px;width: 0px;height: 0px;cursor: pointer;}';
	componentStyle.innerHTML = componentStyleHtml;
	document.body.appendChild(componentStyle);
};

function ToastClass() {}

ToastClass.prototype.show = function (options) {
	if (this.instance) {
		this.hide();
	}
	ToastClass.instance = this;
	let text = options.text || 'LOADING';
	let duration = options.duration;
	this.showFn = options.onShow;
	this.hideFn = options.onHide;
	let type = options.type;

	let toast = document.createElement('div');
	let innerHTMLArr = ['<div class="alertATE-bg"></div><div class="alertATE-wrapper"><div class="alertATE-content">'];
	if (type == "info") {
		innerHTMLArr.push('<div class="alertATE-icon icon-info">');
	} else if (type == "success") {
		innerHTMLArr.push('<div class="alertATE-icon icon-success">');
	} else {
		innerHTMLArr.push('<div class="alertATE-icon icon-error">');
	}
	innerHTMLArr.push('</div><label class="alertATE-msg">' + text + '</label><span class="alertATE-closeBtn" onclick=" ToastClass.instance.hide();"></span></div></div>');
	toast.innerHTML = innerHTMLArr.join('');
	document.body.appendChild(toast);

	// 开启事件
	this.showFn && this.showFn();
	// 如果有全局事件，则执行
	this.onShow && this.onShow();

	this.instance = toast;

	if (duration) {
		setTimeout(function () {
			ToastClass.instance.hide();
		}, duration);
	}

};

ToastClass.prototype.hide = function () {
	let instance = this.instance;
	if (instance && instance.parentNode) {
		instance.parentNode.removeChild(instance);
	}

	// 关闭事件
	this.hideFn && this.hideFn();
	// 如果有全局事件，则执行
	this.onHide && this.onHide();
};
